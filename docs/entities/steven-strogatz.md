---
title: Steven Strogatz
created: 2026-07-31
updated: 2026-07-31
type: entity
classification: person.researcher
domain: biology
tags: [complex-systems, mathematics, theoretical-physics, scaling-laws]
sources: []
confidence: high
status: active
reviewed: 2026-07-31
---

# Steven Strogatz

**Steven Strogatz** is a mathematician at Cornell University known for his work on complex systems, synchronization, and network theory. He is a prominent science communicator and author.

## Key Contributions

- Research on synchronization in complex networks
- Work on small-world networks with Duncan Watts
- Contributions to understanding universal scaling patterns
- Author of "Sync" and "The Joy of x"
- Prominent science communicator

## Connections to Scaling Theory

- His work on network topology connects to fractal network theory
- Collaborated with researchers at the Santa Fe Institute on complex systems
- His mathematics underpins aspects of [wbe-theory](concepts/wbe-theory.md)

## Related

- [wbe-theory](concepts/wbe-theory.md) — Connected through complex systems
- [geoffrey-west](entities/geoffrey-west.md) — Fellow Santa Fe Institute researcher
- [scaling-laws](concepts/scaling-laws.md) — Mathematical framework
- [peter-sheridan-dodds](entities/peter-sheridan-dodds.md) — Related network science researcher
- [fractal-networks](concepts/fractal-networks.md) — Network structure connections

## Related Pages

- [[entities/geoffrey-west]]
- [[entities/peter-sheridan-dodds]]
- [[entities/james-brown]]
