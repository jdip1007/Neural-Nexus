---
title: qPCR Performance Characteristics
created: 2026-08-01
updated: 2026-08-01
type: concept
classification: laboratory.method-evaluation
domain: laboratory
tags: [qpcr, performance-characteristics, analytical-sensitivity, clinical-validation]
sources: []
confidence: low
status: stub
reviewed: 2026-08-01
backlinks: []
---

# qPCR Performance Characteristics

## Overview

Performance metrics for real-time PCR (quantitative PCR) assays in clinical molecular diagnostics.

## Key Metrics

- **Analytical Sensitivity (LOD):** Limit of detection
- **Clinical Sensitivity:** True positive rate in clinical samples
- **Clinical Specificity:** True negative rate
- **Positive Predictive Value (PPV)**
- **Negative Predictive Value (NPV)**
- **Ct Value Precision:** Reproducibility of cycle threshold values
- **Dynamic Range:** Linear quantification range

## Related

- [qpcr](concepts/laboratory-methods/qpcr.md)
- [performance-characteristics](concepts/laboratory-performance/performance-characteristics.md)
- [[limit-of-detection]]
- [[clinical-validation]]

## Related Pages

- [[molecular-diagnostics]]
- [[performance-characteristics]]
- [[qpcr]]
