...
Bridging studies are focused method-evaluation experiments conducted when a validated laboratory test is transferred or modified in a way that does not constitute a wholly new test. The goal is to demonstrate that the modified or transferred method produces results equivalent to the previously validated version, "bridging" the established validation to the new condition without repeating the entire validation package. Bridging studies are commonly performed during reagent lot changes, instrument/platform transfers, protocol modifications, or migration from a validated assay to a new site.
...
- [readings/aphl-verification-validation-toolkit](readings/aphl-verification-validation-toolkit.md)
